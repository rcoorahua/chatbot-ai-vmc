...
| RF-003 | Correo obligatorio únicamente al derivar un usuario anónimo | Cuando un usuario anónimo requiera atención humana, el flujo deberá solicitar correo antes de completar el handoff/ticket | ✅ Hecho 02/09/2026 (D-029) — formulario de asesor con nombre y correo obligatorios y teléfono opcional (`conversations/forms.py`); la conversación anónima se deriva en el sitio y el asesor ve el contacto |
...
| D-019 | Handoff anónimo sin correo | **Revisada por D-029 (02/09/2026)** · **✅ Cerrada 27/08/2026 (derivada de D-002):** no existe handoff anónimo, luego tampoco ticket sin correo. Deja RF-003, AC-003 y RB-002 sin efecto | Media |
...
| D-029 | Casos y handoff con formulario | **✅ Cerrada 02/09/2026 (Aaron):** autenticado = hilo permanente con el bot + hasta 5 casos abiertos (`MAX_OPEN_CASES_PER_USER`) creados por el formulario de asesor (asunto y detalle; correo solo si el JWT no lo trajo); el hilo sigue con el bot encendido. Anónimo = una conversación por sesión (sin localStorage), pide asesor con nombre y correo obligatorios y teléfono opcional, se deriva en el sitio, TTL (`ANONYMOUS_CONVERSATION_TTL_DAYS`) y tope de handoffs por IP y día (`ANON_HANDOFFS_PER_IP_PER_DAY`). El bot ofrece la tarjeta (`HANDOFF_FORM`) en vez de derivar solo. Cierre de caso o de la anónima = `CLOSED` de solo lectura; el hilo del autenticado vuelve al bot. El ticket sigue siendo D-008: el caso es la conversación escalada | Alta |
