...
| 05/09/2026 | **D-031 — anónimo solo FAQ; asesor = crear cuenta (Aaron).** Revisa la parte anónima de D-029 y vuelve a D-002: el visitante tiene una conversación por pestaña y **no deriva ni deja datos**; pedir asesor, quedarse sin evidencia o agotar la cuota responden fijo "necesitas una cuenta en VMC, es gratis" con el botón **Crear cuenta gratis** (URL mock `VMC_SIGNUP_URL`, también en la sesión para la franja del visitante). Se retiran el formulario de contacto, el handoff en el sitio, el `CLOSED` de la anónima, el tope de handoffs por IP y los campos `contact_*`. El formulario del autenticado pasa a un solo paso. **RF-003, AC-003 y RB-002 vuelven a quedar sin efecto** |
...
|---|---|---|
| Usuario anónimo | Persona que usa el chat sin sesión iniciada | FAQ y catálogo; **sin derivación** (D-002, 27/08/2026; confirmado por D-031, 05/09/2026: para un asesor se le manda a crear cuenta) ni historial persistente |
...
| RF-003 | Correo obligatorio únicamente al derivar un usuario anónimo | Cuando un usuario anónimo requiera atención humana, el flujo deberá solicitar correo antes de completar el handoff/ticket | **Sin efecto por D-031 (05/09/2026)**: el anónimo no deriva; se le manda a crear cuenta. (Estuvo implementado del 02/09 al 05/09 por D-029) |
...
| RB-002 | El correo se solicita al anónimo únicamente cuando se necesita handoff humano — **sin efecto por D-002 / D-031** (05/09/2026): el anónimo no deriva, se le manda a crear cuenta |
...
| D-031 | Anónimo solo FAQ; asesor = crear cuenta | **✅ Cerrada 05/09/2026 (Aaron):** el visitante tiene una conversación por pestaña (`sessionStorage`, D-018; fila con TTL) y **solo FAQ**: sin formulario de contacto, sin handoff en el sitio, sin `CLOSED` (si un asesor la tomó y la cierra, vuelve al bot) y sin tope de handoffs por IP. Pedir asesor, quedarse sin evidencia o agotar la cuota (D-027) responden fijo "necesitas una cuenta en VMC, es gratis" con el botón **Crear cuenta gratis** (`interaction.type = LINKS`; URL **mock** `VMC_SIGNUP_URL` hasta que VMC confirme la real, también en `links.signup` de la sesión para la franja del visitante). No se le pregunta "¿te conecto con un asesor?". El formulario del autenticado queda en un solo paso; `contact_*` desaparece de conversaciones, tickets, API del asesor y frontend | Alta |
...
> recibe la pregunta sí/no y el formulario —D-029—; el anónimo, el botón de crear cuenta — D-031).
...
> a asesor; el bot lo manda a crear cuenta (gratis) con un botón. Estuvo vigente del 02/09 al
> 05/09 por D-029. Pendiente retirar este escenario.
