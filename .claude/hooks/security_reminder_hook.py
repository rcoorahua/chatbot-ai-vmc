...
Wire-up: .claude/settings.json (PreToolUse en Edit|Write|MultiEdit).
Adaptacion Subastin: se quito el substring generico ".format(" del patron SQL
(falso positivo masivo en Python; este repo no usa SQL). Resto verbatim.
"""
...
Only use exec() if you absolutely need shell features AND the input is guaranteed to be safe (e.g., from a hardcoded allowlist).""",
    ...
    },
    {
        ...
        "substrings": ["f\"SELECT", "f'SELECT", "f\"INSERT", "f'INSERT", "f\"UPDATE", "f'UPDATE", "f\"DELETE", "f'DELETE"],
