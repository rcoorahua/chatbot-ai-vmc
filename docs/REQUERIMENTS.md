...
| 28/08/2026 | **Observabilidad (RNF-006 parcial).** Logging por stage (`LOG_LEVEL`, `LOG_CONTENT`, `LOG_FORMAT`: dev/stage detallados, prod sin contenido), evento `ai.execution` por decisión del pipeline, ruta `GET /dev/conversations/{id}/ai-usage` (gate `DEV_OBSERVABILITY`, 404 en prod) y consola en `widget/test.html` con capa, modelo, tokens, costo y latencia por mensaje |
...
| 31/08/2026 | Se revisa **D-005**: largo de mensaje baja de 2000 a **500 caracteres** (Aaron). El resto de la decisión (rate limit, límites de imagen, sin tope acumulativo) no cambia |
| 31/08/2026 | Se cierra **D-027** (tope diario de ejecuciones de IA: anónimo 10/día por hash de IP + sesión, autenticado 50/día por `user_id`, sin tope con asesor). **Queda sin implementar** — es la única decisión cerrada con código pendiente; ticket T-09 |
...
| 02/09/2026 | **D-029 — casos y handoff con formulario (Aaron).** Revisa D-002/D-003/D-017/D-019/D-023: el autenticado tiene un hilo permanente con el bot y hasta 5 casos abiertos (cada uno nace del formulario de asesor: asunto + detalle); el anónimo mantiene una sola conversación por sesión y puede pedir asesor dejando nombre y correo (teléfono opcional), con lo que **RF-003 vuelve a tener efecto**; pedir asesor o FAQ sin evidencia ya no derivan solos, el bot ofrece la tarjeta y deriva `POST /chat/conversations/{id}/handoff`; un caso o la conversación anónima cerrados quedan `CLOSED` y de solo lectura. Seguridad del anónimo: TTL en la conversación y sus mensajes, tope de handoffs por IP y día, contacto validado en el servidor. Widget: lista de conversaciones, tarjeta de formulario, sondeo por estado (TD-001) |
| 02/09/2026 | **Módulo de tickets con la taxonomía del corpus (propuesta de Aaron).** `backend/tickets/` implementado: 12 `problem_type` derivados de las 111 preguntas del Centro de Ayuda (MAPEO.md §8) con categoría, prioridad y datos mínimos por tipo; reglas por palabras clave que SUGIEREN el tipo sin llamar a ningún modelo; el asesor confirma o corrige (`classification_source` distingue regla de persona); el ticket sigue a la conversación escalada (PENDING → IN_PROGRESS → CLOSED) y se cierra con `resolution`. Rutas `/advisor`: `GET /taxonomy`, `GET /tickets`, `GET /conversations/{id}/ticket`, `PATCH /tickets/{id}`. ⚠️ **D-008 SIGUE ABIERTA**: esto es el insumo implementado y medible, no la decisión — la cierran Silvana + Julio |
| 02/09/2026 | **Continuidad conversacional y confirmación antes de derivar (Aaron, tras ver el flujo real).** (1) `agent/followups.py`: si el mensaje es una continuación ("ya estoy ahí", "y luego"), la consulta al RAG se arma con la pregunta previa del usuario. El bot explicaba el registro paso a paso y al responder "ya estoy ahí" derivaba por falta de evidencia teniendo el artículo correcto entre los descartados (0.789 vs. umbral 0.84); verificado en el índice real, los seguimientos pasan de 0/4 a 4/4 sobre el umbral. Reglas deterministas, sin llamada a modelo. (2) **Revisión de D-029**: sin evidencia el bot PREGUNTA "¿te conecto con un asesor?" (botones sí/no, flujo `HANDOFF_CONFIRM`) y el formulario sale solo con el "sí"; quien PIDE un asesor lo sigue recibiendo directo |
...
| 05/09/2026 | **D-031 — anónimo solo FAQ y solo el bot; el asesor solo por la conversación (Aaron, ajustada el 06/09).** Revisa la parte anónima de D-029 y el badge de D-030, y vuelve a D-002: el visitante tiene una conversación por pestaña, **no deriva ni deja datos** y **ningún asesor puede tomarla** (409). Pedir asesor o decir que sí a "¿deseas contactar a un asesor?" responde con un mensaje amable que lo manda a **iniciar sesión** con el botón **Iniciar sesión** (URL mock `VMC_LOGIN_URL`, también en la sesión para la franja del visitante). **Sin botón de asesor en la UI**: se retira el badge "Asesor humano"; el agente solo sugiere al asesor dentro de la conversación (último mensaje sugerido **Contactar asesor** bajo cada respuesta, reconocido por estructura y sin modelo, y la pregunta sí/no sin evidencia). Se retiran también el formulario de contacto, el handoff en el sitio, el `CLOSED` de la anónima, el tope de handoffs por IP y los campos `contact_*`. El formulario del autenticado pasa a un solo paso. **RF-003, AC-003 y RB-002 vuelven a quedar sin efecto** |
...
|---|---|---|
| Usuario anónimo | Persona que usa el chat sin sesión iniciada | FAQ y catálogo, atendido solo por el bot; **sin derivación** (D-002, 27/08/2026; confirmado por D-031, 05/09/2026: para un asesor se le manda a iniciar sesión) ni historial persistente |
...
| RF-003 | Correo obligatorio únicamente al derivar un usuario anónimo | Cuando un usuario anónimo requiera atención humana, el flujo deberá solicitar correo antes de completar el handoff/ticket | **Sin efecto por D-031 (05/09/2026)**: el anónimo no deriva; se le manda a iniciar sesión. (Estuvo implementado del 02/09 al 05/09 por D-029) |
...
| RF-014 | Límites configurables contra abuso | El sistema deberá soportar límites de cantidad de mensajes, longitud, frecuencia, imágenes y tamaño de conversación. Los valores se cierran en D-005 | ✅ Hecho 28/08/2026 — D-005 cerrada: largo de mensaje (500, revisado 31/08 desde 2000), rate limit (10/min → 429) y límites de imagen configurados. La aplicación de los de imagen es F6 |

## 3.3 IA, clasificación y FAQ/RAG

| ID | Requerimiento funcional | Criterio / comportamiento esperado | Estado |
|---|---|---|---|
| RF-015 | Haiku como capa de lectura y orquestación | Los mensajes elegibles deberán ser clasificados para decidir ruta de respuesta sin usar Gemini como clasificador general | ✅ Hecho 28/08/2026 — reglas deterministas primero (`agent/heuristics.py`, costo cero) y el resto al tier FAST. **Por TD-008 hoy Gemini (tier FAST de `core/llm.py`; 2026-09-01: `gemini-3.5-flash-lite`) también orquesta**; Haiku sigue siendo el plan B del tier si el golden set muestra que el routing no alcanza. **Golden set** en `tests/golden/intents.jsonl` (D-026): la parte determinista corre en CI; la eval real contra Gemini es `python -m scripts.eval_intents` (piso 95%) |
...
| RF-017 | FAQ basada en conocimiento VMC recuperado | Las respuestas FAQ se generarán a partir de documentos/contenido recuperado desde Pinecone y del system prompt autorizado | ✅ Hecho 28/08/2026 — pipeline conectado: el worker recupera de Pinecone y redacta solo con esa evidencia. `RAG_MIN_SCORE` calibrado en `0.84` contra el índice real (antes `0.75`, dejaba pasar preguntas ajenas a VMC como evidencia — ver CLAUDE.md "RAG"); margen angosto, revisar si el corpus crece mucho |
...
| RF-019 | Fuentes/enlaces cuando estén disponibles | La respuesta automática deberá incluir el enlace al centro de ayuda o fuente recuperada cuando exista | ✅ Hecho 28/08/2026; revisado 03/09/2026 (D-030) — la fuente ya no va en el texto de la respuesta: viaja en `metadata.sources` del mensaje del bot y el widget la muestra como chip bajo la burbuja, determinista desde la evidencia (el modelo no escribe enlaces) |
| RF-020 | Gemini como capa de escritura | Gemini recibirá el mensaje, contexto reciente y fragmentos recuperados para redactar la respuesta automática. No procesa los casos que ya pasan directamente a atención humana | ✅ Hecho 28/08/2026 — el tier ANSWER (2026-09-01: `gemini-3.6-flash`) redacta con el mensaje, la ventana de contexto (D-004) y los fragmentos; los casos que derivan directo no pasan por él |
...
| RF-023 | Ticket solo cuando existe atención humana | Una conversación automática puede existir sin ticket. El ticket representa trabajo que requiere intervención de asesor | ✅ Hecho 02/09/2026 — el ticket nace con el handoff (D-029) y solo entonces; una conversación que el bot resuelve no genera ninguno (`GET /advisor/conversations/{id}/ticket` responde 404). 1:1 con la conversación escalada |
| RF-024 | Recolección previa de datos requeridos por tipo de ticket | Antes de crear ciertos tickets, el bot podrá solicitar datos mínimos del caso. La taxonomía y campos se definen en D-008 | Parcial ✅ 02/09/2026 — el formulario de handoff (D-029) recoge asunto, detalle y contacto, y cada `problem_type` declara sus datos mínimos: el ticket expone `missing_data` (lo que el asesor debe pedir) y `collected_data` (lo que ya registró, vía `PATCH`). **Pendiente:** que el BOT los pida en el chat antes de crear el ticket — depende de los campos obligatorios definitivos (D-008) |
...
| RF-033 | Vista de conversación e información contextual | El asesor podrá ver el hilo y datos disponibles/autorizados del usuario, como nombre, correo, empresa, identificadores y relaciones relevantes. Los campos definitivos se validan en D-010 | Parcial ✅ 27/08/2026 — hilo (últimos 20) + los campos que ya guarda la conversación (nombre, correo, empresa, id VMC). D-010 cerrada 07/09/2026: CUU + id VMC + correo del JWT (el nombre lo asigna el asesor) |
...
| RNF-006 | Observabilidad | Logs, métricas y alertas | CloudWatch u observabilidad equivalente — Parcial ✅ 28/08/2026: logs estructurados por stage (`core/observability.py`: dev/stage DEBUG con vista previa, prod INFO sin contenido; JSON en Lambda), evento `ai.execution` por decisión del pipeline con las mismas claves que AIUsage, y consola de dev en `widget/test.html` sobre `GET /dev/conversations/{id}/ai-usage` (404 en prod). Log de peticiones HTTP desde el 02/09/2026 (`api/request_log.py`): ruta, estado, duración y el motivo de cada rechazo, con el nivel según el resultado. Métricas y alarmas de CloudWatch esperan la cuenta AWS |
...
| RB-002 | El correo se solicita al anónimo únicamente cuando se necesita handoff humano — **sin efecto por D-002 / D-031** (05/09/2026): el anónimo no deriva, se le manda a iniciar sesión |
...
| D-002 | Máximo de conversaciones activas para usuario autenticado | **Revisada por D-029 (02/09/2026); la parte anónima vuelve a regir por D-031 (05/09/2026)** · **✅ Cerrada 27/08/2026:** 1 conversación activa (autenticado y anónimo). Máximo 5 tickets activos por usuario. El anónimo solo recibe FAQ: sin handoff ni ticket, porque no hay forma de identificarlo para continuar un ticket días después | Alta |
| D-003 | Cierre, reapertura e historial visible | **Revisada por D-029 (02/09/2026)** · **✅ Cerrada 27/08/2026:** una sola conversación permanente por usuario autenticado; el historial son los tickets, que al cerrarse dejan la nota "Ticket cerrado" en el hilo (como Intercom). Sin autocierre de conversación; el ticket sin respuesta se decide en D-007 | Alta |
...
| D-008 | Taxonomía de problemas y tickets | Definir tipos de problema, qué genera ticket, campos obligatorios por tipo, área responsable, prioridad y criterios de cierre. **Propuesta de Aaron IMPLEMENTADA 02/09/2026** (`backend/tickets/taxonomy.py`, detalle en MAPEO.md §8): 12 `problem_type` derivados del corpus con categoría, prioridad y datos mínimos, más las etiquetas de D-009. Corre en producción de dev y el asesor puede corregir cada ticket, así que la decisión se puede tomar con datos en vez de en abstracto. **Falta para cerrarla:** validar la lista contra los motivos reales que hoy ve el equipo en Intercom, fijar los campos obligatorios por tipo y el SLA de cada prioridad | Alta |
...
| D-010 | Campos de usuario visibles y utilizables | **CERRADA 07/09/2026 (Aaron)**: del JWT llegan `user_id`, `email` y `cuu` (el código que VMC le muestra al usuario). Se guardan en la conversación y en el ticket, y el asesor los ve; el CUU es solo para mostrar (sin búsqueda ni GSI) y el nombre lo asigna el KAM, no el JWT | Alta |
...
| D-017 | Relación conversación ↔ ticket | **Revisada por D-029 (02/09/2026)** · **✅ Cerrada 27/08/2026 (derivada de D-002/D-003):** varios tickets por conversación (máx. 5 activos); cerrar un ticket no cierra la conversación | Alta |
...
| D-019 | Handoff anónimo sin correo | **Revisada por D-029 (02/09/2026) y de vuelta en vigor por D-031 (05/09/2026)** · **✅ Cerrada 27/08/2026 (derivada de D-002):** no existe handoff anónimo, luego tampoco ticket sin correo. Deja RF-003, AC-003 y RB-002 sin efecto | Media |
...
| D-023 | Cierre de caso sin ticket | **Revisada por D-029 (02/09/2026)** · **✅ Cerrada 27/08/2026 (Aaron), provisional hasta F5:** el asesor asignado cierra con `POST …/close`: nota `TICKET_CLOSED` en el hilo, conversación de vuelta a `BOT_ATTENDING` con el bot encendido y sin asesor (D-003: la conversación no se cierra). No se crea fila en Tickets; cuando exista el módulo, el cierre pasará a cerrar el ticket | Alta |
| D-027 | Tope diario de ejecuciones de IA | **✅ Cerrada 31/08/2026 (Aaron) — ⚠️ SIN IMPLEMENTAR, ticket T-09:** **anónimo 10 ejecuciones de IA al día**, contadas por `hash(IP)` **y** por sesión (se agota la primera de las dos); **autenticado 50/día** por `user_id`; **sin tope mientras atiende un asesor** (no hay llamada a modelo). `0 = ilimitado` (dev). Cuenta la ejecución de IA, no el mensaje: triviales y guardrails no gastan cuota. Complementa a D-005 (que es por minuto y por conversación) frenando el costo acumulado de un mismo actor a lo largo del día. **Es la única decisión cerrada con código pendiente y debe implementarse apenas se pueda**: hoy un anónimo con un script solo topa contra el límite de 10/min, o sea 14 400 llamadas al día. Al construirlo: la IP se guarda **hasheada** (dato personal) y hay que asumir que CGNAT móvil hace que usuarios legítimos compartan IP | Alta |
| D-029 | Casos y handoff con formulario | **Parte anónima revisada por D-031 (05/09/2026)** · **✅ Cerrada 02/09/2026 (Aaron):** autenticado = hilo permanente con el bot + hasta 5 casos abiertos (`MAX_OPEN_CASES_PER_USER`) creados por el formulario de asesor (asunto y detalle; correo solo si el JWT no lo trajo); el hilo sigue con el bot encendido. ~~Anónimo = una conversación por sesión (sin localStorage), pide asesor con nombre y correo obligatorios y teléfono opcional, se deriva en el sitio, TTL (`ANONYMOUS_CONVERSATION_TTL_DAYS`) y tope de handoffs por IP y día (`ANON_HANDOFFS_PER_IP_PER_DAY`).~~ El bot ofrece la tarjeta (`HANDOFF_FORM`) en vez de derivar solo. Cierre de caso = `CLOSED` de solo lectura; un hilo con el bot vuelve al bot. El ticket sigue siendo D-008: el caso es la conversación escalada | Alta |
| D-031 | Anónimo solo FAQ y solo el bot; asesor solo por la conversación | **✅ Cerrada 05/09/2026, ajustada 06/09 (Aaron):** el visitante tiene una conversación por pestaña (`sessionStorage`, D-018; fila con TTL) y **solo FAQ**: sin formulario de contacto, sin handoff en el sitio, sin `CLOSED`, sin tope de handoffs por IP y **sin asesor que la tome** (`/take` → 409, tampoco por intervención proactiva). Pedir asesor, o decir que sí a "¿deseas contactar a un asesor?", o agotar la cuota (D-027) responden fijo con la invitación a **iniciar sesión** y el botón **Iniciar sesión** (`interaction.type = LINKS`; URL **mock** `VMC_LOGIN_URL` hasta que VMC confirme la real, también en `links.login` de la sesión para la franja del visitante). **Sin botón de asesor en la UI**: se retira el badge "Asesor humano" de D-030; el agente solo sugiere al asesor dentro de la conversación —último mensaje sugerido **Contactar asesor** bajo cada respuesta con evidencia, cuyo clic se reconoce por estructura sin clasificador ni modelo, y la pregunta sí/no sin evidencia, igual para anónimo y autenticado—; "quiero un asesor" escrito pasa por el orquestador y termina igual. El formulario del autenticado queda en un solo paso y solo lo ofrece el bot; `contact_*` desaparece de conversaciones, tickets, API del asesor y frontend | Alta |
...
> ✅ **Cubierto 28/08/2026** por `tests/test_ai_worker.py` (sin evidencia: el autenticado
> recibe la pregunta sí/no y el formulario —D-029—; el anónimo, la misma pregunta y, con el
> sí, el botón de iniciar sesión — D-031).
...
> **Sin efecto (D-002, 27/08/2026; confirmado por D-031, 05/09/2026):** el anónimo no deriva
> a asesor; el bot lo manda a iniciar sesión con un botón. Estuvo vigente del 02/09 al 05/09
> por D-029. Pendiente retirar este escenario.
